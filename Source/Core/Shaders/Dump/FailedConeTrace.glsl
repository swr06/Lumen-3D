...
void main() {
    ...
    float ConeTheta = SpecularPowerToConeAngle(SpecularPower) / 2.0f;
    
    float AdjacentLength = pow(Transversal, 1.0f);
    ...
    for (int i = 0 ; i < 7 ; i++) {
        ...
        vec2 SampleUV = v_TexCoords.xy + AdjacentUnit * 0.1 * (AdjacentLength - InscribedCircleSize);
